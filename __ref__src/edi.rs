    ...
    *******************************************************************/
    fn decode_frame(&mut self, edi_frame: &[u8]) {
        ...
        while i < len.saturating_sub(8) {
            ...
            if tag_name == "*ptr" {
                // Control TAG Item "Protocol Type and Revision"
                if tag_len != 64 {
                    eprintln!(
            ...
            if tag_name == "deti" {
                // EDI Packet Counter, Timing and ETI Header information and FIC.
            ...
            if tag_name.starts_with("est") && tag_item[3] >= 1 && tag_item[3] <= 64 {
                // The data for one MSC stream for one logical frame
                if tag_len < 3 * 8 {
                    eprintln!(
